...
#define buzzerPin 7   // Piezo Buzzer->D7
...
void setup(void)
{
      ...
      pinMode(buzzerPin, OUTPUT); // Set buzzer Pin as output mode
...
void playSoundBasedOnDistance(int distance) {
   ...
   if (distance < 30) {  // If object is very close
       toneFrequency = 2000; // High tone
   } else if (distance < 100) { // If object is moderately close
       toneFrequency = 1000; // Medium tone
   } else {
       toneFrequency = 500; // Low tone
   }
   
   tone(buzzerPin, toneFrequency, 100); // Play tone for 100 milliseconds
   delay(100); // Delay to avoid continuous sound
}
...
void loop(void)
{
  ...
  for (int x=180; x > 4; x-=2){       //The base servo rotates from 180 to 0 degrees
   ...
   playSoundBasedOnDistance(distance);
